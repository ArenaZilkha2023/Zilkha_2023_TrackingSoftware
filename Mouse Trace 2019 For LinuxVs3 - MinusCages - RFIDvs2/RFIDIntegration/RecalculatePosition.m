function  [IndexMouse,xNew,yNew,xNewPixel,yNewPixel,OrientationNew]=RecalculatePosition(recordsNoDefined,XAssignment,YAssignment,...
                                                                    xpos,ypos,xposPixel,yposPixel,Orientation,AntennaX,AntennaY,MaxDistanceToleranceRFID);
IndexMouse=[];
xNew=[];
yNew=[];
xNewPixel=[];
...
x=XAssignment(find(XAssignment~=1e6)); % positions which were assigned
y=YAssignment(find(YAssignment~=1e6)); % positions which were assigned
...
AntennaX=AntennaX(recordsNoDefined);
AntennaY=AntennaY(recordsNoDefined);                                                               
 % %%%%%%%%%%%%%%%%Arrange distance from antenna into a matrix%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 

if ~isempty(xnoassign)
...
% else 
%     for countn=1:length(recordsNoDefined)
%                xNew(countn)=1e+6;
%                 yNew(countn)=1e+6;
%                 xNewPixel(countn)=1e+6;
%                 yNewPixel(countn)=1e+6;
%                 OrientationNew(countn)=1e+6;
     end                                                           
end

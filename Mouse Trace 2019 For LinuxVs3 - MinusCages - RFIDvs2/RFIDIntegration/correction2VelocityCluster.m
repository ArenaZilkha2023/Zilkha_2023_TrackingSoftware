function Locomotion=correction2VelocityCluster(Locomotion,VelocityThresholForCorrection,Conversionx,Conversiony,TotalFrames,IsSleeping)
...
Logical_forlargervelocity=or((vel>VelocityThresholForCorrection & vel<1e6)&(Locomotion.AssigRFID.XcoordMM~=1e6),(isnan(vel))&(Locomotion.AssigRFID.XcoordMM==1e6));
...
for countr=1:length(frames)
   % step 4)-
         countr
          ...
          if length(Index)==2 
              counti=1;
              % Find clusters of the two mice either from 2 or 3 cluster
              Ic2=VectorCluster(1:frames(countr)-1,Index(1))==2 & VectorCluster(1:frames(countr)-1,Index(2))==2;
              Ic3=VectorCluster(1:frames(countr)-1,Index(1))==3 & VectorCluster(1:frames(countr)-1,Index(2))==3;
...
%                       if isempty(find(VectorCluster(ifinishcluster+1:frames(countr)-1,Index(2))==2 & VectorCluster(ifinishcluster+1:frames(countr)-1,Index(2))==3)) 
              ...
              if ~any(or(vel(ifinishcluster+1:frames(countr)-1,Index(1))>VelocityThresholForCorrection,vel(ifinishcluster+1:frames(countr)-1,Index(2))>VelocityThresholForCorrection)) 
                  % only consider cases inside the arena
                    if ~any(or(IsSleeping(ifinishcluster+1:frames(countr)-1,Index(1))==1,IsSleeping(ifinishcluster+1:frames(countr)-1,Index(2))==1)) 

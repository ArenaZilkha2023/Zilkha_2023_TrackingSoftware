function [rowNumber,FinalData] = munkresForMiceTracer(costMat) %gives for each antenna the adequate number of position and also the distance to the respective antenna
...
%get data from the assigment that is real without the data added for doing
%square the matrix
countnew=1;
for countaux=1:length(assignment)
    if assignment(countaux)~=0
    colNumber(countnew)=assignment(countaux);
    rowNumber(countnew)=countaux;
...
%sort
[~,isort]=sort(colNumber);
colNumber=colNumber(isort);
rowNumber=rowNumber(isort);

for j=1:length(rowNumber)
FinalData(j)=costMat(rowNumber(j),colNumber(j));
end
